...
function normaliseTags(tags: PatternTag[]): PatternTag[] {
  return [...new Set(tags.map((t) => t.trim().toUpperCase()))].sort();
}
...
function patternMatches(
  rule: GlobalVetoRule,
  candidate: { tags: PatternTag[]; contract?: string; entryDigit?: number | null; symbol?: string },
): boolean {
  const ruleTags = normaliseTags(rule.pattern.tags);
  const candidateTags = normaliseTags(candidate.tags);
  ...
  if (rule.scope === "MARKET") {
    if (!candidate.symbol || candidate.symbol !== rule.symbol) return false;
  }
  return true;
}
...
export function evaluateSignalGovernance(candidate: VetoCandidate): SignalGovernanceResult {
  const vetoCheck = checkGlobalVeto(candidate);
  const risk = patternRiskStats({ tags: candidate.tags });
