...
export function computePriceActionField(
  ...
  for (let d = 0; d < 10; d++) {
    ...
    if (!measurable) {
      ...
      movement = "WEAKENING";
    } else if (Math.sign(pctMid - pct) !== 0 && Math.sign(pctFast - pctMid) === -Math.sign(pctMid - pct) && Math.abs(accel) >= 1.2) {
      movement = "REVERSING";
...
export interface ContractPriceAction {
  ...
  specialRisk: { digit: number; reading: DigitPressureReading | null; elevated: boolean; note: string };
...
export function assessLosingSideTakeover(
  ...
  if (worst && worst.accelerationPp > 0.8) {
    ...
    evidence.push(`Losing digit ${worst.digit} is accelerating (+${worst.accelerationPp.toFixed(2)}pp).`);
  }
  if (worst && worst.persistence >= 1) {
    ...
    severity += 4;
    evidence.push(`Losing digit ${worst.digit} spiked in the fast subwindow — burst rather than sustained.`);
  }
...
export function evaluateContractPriceAction({
  ...
  if (field.measurable) {
    ...
    } else if (parityLosing.direction === "INCREASING" && parityWinning.direction === "DECREASING") {
      cautions.push(
  ...
  if (field.measurable) {
    ...
    else if (winningSide.direction === "INCREASING" && losingSide.direction === "STABLE") delta += 1.5;
    else if (winningSide.direction === "DECREASING" && losingSide.direction === "INCREASING")
  ...
  const veto = field.measurable && takeover.state === "CONFIRMED TAKEOVER" && takeover.severity >= 75;
