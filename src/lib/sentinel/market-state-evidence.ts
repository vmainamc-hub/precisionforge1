...
export function buildEvidenceProfile(
  ...
  if (n < minSample) {
    ...
    if (regime === "TRANSITION" || regime === "CHOPPY") {
      ...
      verdict =
        longWindow.n >= minSample * 2 && !changePoint.detected ? "CONFIRMED_EDGE" : "DEVELOPING_EDGE";
    ...
    const stabilityBonus = stability.label === "STABLE" ? 15 : stability.label === "TRENDING" ? 8 : 0;
